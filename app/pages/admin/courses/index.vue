<template>
  <div class="page">
    <AdminAppSkeletonLoader v-if="loading" :rows="8" :cols="6" />
    <template v-else>
      <!-- Header -->
      ...
      >
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <select v-model="form.subjectGroup" class="form-input">
              <option value="">-- เลือกกลุ่มสาระ --</option>
              <option v-for="g in groupOptions" :key="g" :value="g">{{ g }}</option>
            </select>
            <span v-if="formErrors.subjectGroup" class="field-error">{{ formErrors.subjectGroup }}</span>
          </div>
          <div class="form-group">
          ...
          <div class="form-group form-group--full">
            ...
            <input v-model="form.teacherName" class="form-input" placeholder="ชื่อครูผู้สอน" />
          </div>
          <div class="form-group form-group--full">
...
const groupOptions = computed(() => subjectGroupRows.value.map(r => r.name))
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter(r => {
    ...
    if (filterGroup.value && r.subjectGroup !== filterGroup.value) return false
...
const emptyForm = (): CourseRow => ({
  id: 0, code: '', name: '', subjectGroup: '', level: 'ม.1',
  credits: 1.5, hoursPerWeek: 3, semester: '1', year: '2568',
  teacherName: '', description: '',
})
const form = ref<CourseRow>(emptyForm())
const formErrors = ref({ code: '', name: '', subjectGroup: '' })

function validate() {
  formErrors.value = { code: '', name: '', subjectGroup: '' }
  let ok = true
  ...
  if (!form.value.subjectGroup) { formErrors.value.subjectGroup = 'กรุณาเลือกกลุ่มสาระ'; ok = false }
  return ok
}
